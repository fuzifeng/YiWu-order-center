...
public class HandlerProccessor implements BeanFactoryPostProcessor {
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        ...
        for (OrderTypeEnum temp : OrderTypeEnum.values()) {
            AOrderTypeHandler beanInstacle = getBeansWithAnnotation(beanFactory, AOrderTypeHandler.class,
                    OrderTypeHandlerAnnotation.class, temp.getCode());
    ...
    private <T> T getBeansWithAnnotation(ConfigurableListableBeanFactory beanFactory, Class<T> manager,
                                         Class<? extends OrderTypeHandlerAnnotation> annotation, String code) throws BeansException {
        if (ObjectUtils.isEmpty(code)) {
            throw new RuntimeException("code is null ");
        }
        Collection<T> tCollection = beanFactory.getBeansOfType(manager).values();
        for (T t : tCollection) {
            OrderTypeHandlerAnnotation orderTypeHandlerAnno = t.getClass().getAnnotation(annotation);
